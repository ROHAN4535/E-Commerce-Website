...
const CartProvider = (props) => {
  ...
  const fetchCartItemsFromBackend = async () => {
    try {
      ...
      const response = await axios.get(
        `https://crudcrud.com/api/1dc62c8168904e2384b71a51fcde0897/cart${email}`
      );
  ...
  useEffect(() => {
    fetchCartItemsFromBackend();  // eslint-disable-next-line 
  }, [authCtx.userEmail]);
  ...
  const removeItemFromCart = async (index) => {
    try {
      ...
      if (backendId) {
        await axios.delete(
          `https://crudcrud.com/api/1dc62c8168904e2384b71a51fcde0897/cart${email}/${backendId}`
        );
